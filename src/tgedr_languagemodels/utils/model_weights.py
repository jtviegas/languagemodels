"""Module for loading pre-trained GPT model weights into a model instance.

This module provides functionality to assign pre-trained weights from a params
dictionary into a GPT model, including embeddings, transformer blocks, attention
layers, feed-forward networks, and normalization layers.
"""
...
def load_weights_into_gpt(gpt, params):  # 1
    ...
    for b in range(len(params["blocks"])):  # 2
        ...
        gpt.trf_blocks[b].att.out_projection.weight = assign(
            gpt.trf_blocks[b].att.out_projection.weight, params["blocks"][b]["attn"]["c_proj"]["w"].T
        )
        gpt.trf_blocks[b].att.out_projection.bias = assign(
            gpt.trf_blocks[b].att.out_projection.bias, params["blocks"][b]["attn"]["c_proj"]["b"]
        )
